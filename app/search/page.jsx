...
export default function Search(){
  ...
  useEffect(() => {
    let isMounted = true
    let timerId;
    if(isMounted && isError && status !== 404 && status !== 401 && reload <= 5) {
      timerId = setTimeout(() => {
...
// maxscreen:grid-cols-1 ${sortType === 'GRID' ? 'grid-cols-2' : 'grid-cols-1'} gap-x-3 gap-y-3.5
  return (
    <main 
      ...
      {/* search results */}
        <section className={`flex-auto shadow-inner flex flex-col gap-2.5 shadow-slate-200 min-h-[25rem] transition-all p-1 py-2 w-full`}>
         {searchContent}
        </section>
